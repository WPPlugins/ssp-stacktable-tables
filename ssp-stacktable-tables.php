...
function ssp_stacktable_add_scripts() {
	...
	if(in_array('all', $pages) || (is_a( $post, 'WP_Post' ) && in_array($post->ID, $pages))) {
...
function ssp_stactable_admin_scripts($hook) {
	...
	wp_enqueue_style('ssp-select2', plugins_url('select2.min.css', __FILE__ ), array(), '4.0.3');
	wp_enqueue_script('ssp-select2', plugins_url('select2.min.js', __FILE__ ), array('jquery'), '4.0.3');
...
function ssp_stacktable_pages_select() {
    ...
    $values = array( 'none' => __('None (disabled)', 'ssp-stacktable-tables' ), 'all' => __('ALL Pages and Posts', 'ssp-stacktable-tables'));
    foreach ($ids as $id) {
    ...
    <br/>
    <em><?php _e('Select one or more pages where you want the script to be enqueued. Stacktable formatting will apply to ALL tables on the selected pages.<br/>Select the ALL option to globally load the script on ALL pages of your site.<br/>Select the None option to disable the script everywhere (or just deactivate the plugin).'); ?></em>
    <?php
}
